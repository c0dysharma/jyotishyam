...
from generic.panchanga import PanchangaData
...
class General:
    ...
    vaara_names = ["Sunday", "Monday", "Tuesday",
    ...
    def signnum(self, signstr): return self.signs.index(signstr) + 1

    ###############################################################################
    ##                                 APIs                                      ##
    ###############################################################################

    def update_miscdata(self, jd, place, miscdata):
        # update maasa
        (maasanum, leap) = self.panchanga.masa(jd, place)
        if (leap == True):
            miscdata["maasa"] = "adhikamaasa - " + \
                self.maasa_names[maasanum - 1]
        else:
            miscdata["maasa"] = self.maasa_names[maasanum - 1]

        miscdata["vaara"] = self.vaara_names[self.panchanga.vaara(jd)-1]

        miscdata["tithi"] = self.tithi_names[self.panchanga.tithi(jd, place)[
            0] - 1]
        print(self.panchanga.jd_to_gregorian(jd))
        print(place)
        print(self.panchanga.tithi(jd, place))

        return

    def housediff(self, fromsign, tosign):
    ...
    def compute_nthsign(self, fromsign, n):
        s = (fromsign + n - 1) % 12
        if (s == 0):
            s = 12
        return (s)

    def compute_nthsign_backwards(self, fromsign, n):
        s = (12 + fromsign - n + 1) % 12
        if (s == 0):
            s = 12
        return (s)

    def compute_aspects4planet(self, planetname, aspectnum, division):
        ...
        if (planet != "NOT_FOUND"):
            ...
            signno = self.signnum(planet["sign"])
            aspecthousenum = self.compute_nthsign(houseno, aspectnum)
            planet["Aspects"]["houses"].append(
                aspecthousenum)  # nth house aspect
            planet["Aspects"]["signs"].append(
                self.signs[self.compute_nthsign(signno, aspectnum)-1])  # nth sign aspect
            planets = self.get_planets_in_house(
                aspecthousenum, division["planets"])
            for p in planets:
    ...
    def compute_aspects(self, division):
        ''' Computes the aspects of each planet in planet group and 
            updates its Aspects elements - planets, signs and houses '''
        # Sun aspects 7th house
        self.compute_aspects4planet("Sun", 7, division)
        # Moon aspects 7th house
        self.compute_aspects4planet("Moon", 7, division)
        # Mars aspects 4th house
        self.compute_aspects4planet("Mars", 4, division)
        # Mars aspects 7th house
        self.compute_aspects4planet("Mars", 7, division)
        # Mars aspects 8th house
        self.compute_aspects4planet("Mars", 8, division)
        # Mercury aspects 7th house
        self.compute_aspects4planet("Mercury", 7, division)
        # Jupiter aspects 5th house
        self.compute_aspects4planet("Jupiter", 5, division)
        # Jupiter aspects 7th house
        self.compute_aspects4planet("Jupiter", 7, division)
        # Jupiter aspects 9th house
        self.compute_aspects4planet("Jupiter", 9, division)
        # Venus aspects 7th house
        self.compute_aspects4planet("Venus", 7, division)
        # Saturn aspects 3rd house
        self.compute_aspects4planet("Saturn", 3, division)
        # Saturn aspects 7th house
        self.compute_aspects4planet("Saturn", 7, division)
        # Saturn aspects 10th house
        self.compute_aspects4planet("Saturn", 10, division)
        # Rahu aspects 5th house
        self.compute_aspects4planet("Rahu", 5, division)
        # Rahu aspects 7th house
        self.compute_aspects4planet("Rahu", 7, division)
        # Rahu aspects 9th house
        self.compute_aspects4planet("Rahu", 9, division)
        # Ketu aspects 5th house
        self.compute_aspects4planet("Ketu", 5, division)
        # Ketu aspects 7th house
        self.compute_aspects4planet("Ketu", 7, division)
        # Ketu aspects 9th house
        self.compute_aspects4planet("Ketu", 9, division)
        return

    def compute_aspectedby(self, division):
    ...
    def compute_conjuncts(self, division):
    ...
    def populate_kendraplanets(self, division):
    ...
    def populate_trikonaplanets(self, division):
    ...
    def populate_trikplanets(self, division):
    ...
    def populate_upachayaplanets(self, division):
    ...
    def populate_dharmaplanets(self, division):
    ...
    def populate_arthaplanets(self, division):
    ...
    def populate_kamaplanets(self, division):
    ...
    def populate_mokshaplanets(self, division):
    ...
    def get_planets_in_house(self, houseno, planetgroup):
        houseplanets = []
        for planetname in planetgroup:
    ...
    def update_houses(self, division):
        ...
        for housenum in range(1, 13):  # house 1 to 12
            ...
            planets = self.get_planets_in_house(housenum, division["planets"])
            ...
            mysignnum = self.compute_nthsign(
                self.signnum(division["ascendant"]["sign"]), housenum)
            house["sign-num"] = mysignnum
            house["sign"] = self.signs[mysignnum - 1]
            house["sign-lord"] = self.signlords[mysignnum - 1]
            house["rashi"] = self.rashis[mysignnum - 1]

            # compute planets which are aspecting this house
            # all planets will have 7th aspect.So add planets in 7th from it
            planets = self.get_planets_in_house(
                self.compute_nthsign_backwards(housenum, 7), division["planets"])
            for planet in planets:
                house["aspect-planets"].append(planet)
            # Mars has 4th aspect. so check if mars is present in 4 house backwards
            planets = self.get_planets_in_house(
                self.compute_nthsign_backwards(housenum, 4), division["planets"])
            if ("Mars" in planets):
                house["aspect-planets"].append("Mars")
            # Mars has 8th aspect. so check if mars is present in 8 house backwards
            planets = self.get_planets_in_house(
                self.compute_nthsign_backwards(housenum, 8), division["planets"])
            if ("Mars" in planets):
                house["aspect-planets"].append("Mars")
            # Saturn has 3rd aspect. so check if saturn is present in 3 house backwards
            planets = self.get_planets_in_house(
                self.compute_nthsign_backwards(housenum, 3), division["planets"])
